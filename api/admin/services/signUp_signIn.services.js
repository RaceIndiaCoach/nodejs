...
exports.registerService = async (data, callback) => {
    try {
        let { mobile_no, email, role } = data.body;
        let mobileNovalidation = await db.user.findOne({ where: { mobile_no: mobile_no }});
...
exports.loginService = async (data, callback) => {
    try {
        let { mobile_no, email, password } = data.body;
        let mobileNovalidation = await db.user.findOne({ where: { mobile_no: mobile_no }});
        if (!mobileNovalidation) {
            return callback(null, {
        ...
        //     });
        // }
        let userPassComapre = await bcrypt.compare(password, mobileNovalidation.password);
        if (!userPassComapre) {
            return callback(null, {
        ...
        return callback(null, {
            message: 'Welcome',
            _id: mobileNovalidation._id,
            name: mobileNovalidation.name,
            role: mobileNovalidation.role,
            token: token,
            status_code: 200 
        });
    } catch (error) {
